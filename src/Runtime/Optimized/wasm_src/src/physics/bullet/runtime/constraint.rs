...
impl Generic6DofConstraint {
    ...
    pub(crate) fn use_frame_offset(&mut self, frame_offset_on_off: bool) {
        self.inner.use_frame_offset(frame_offset_on_off);
    }
}
...
#[wasm_bindgen(js_name = "constraintUseFrameOffset")]
pub fn constraint_use_frame_offset(ptr: *mut usize, frame_offset_on_off: bool) {
    let constraint = unsafe { &mut *(ptr as *mut Constraint) };

    match constraint {
        Constraint::Generic6Dof(constraint) => {
            constraint.use_frame_offset(frame_offset_on_off);
        },
        Constraint::Generic6DofSpring(constraint) => {
            constraint.use_frame_offset(frame_offset_on_off);
        },
        _ => {
            panic!("constraintUseFrameOffset: use_frame_offset is not supported for this constraint type");
        }
    }
}
