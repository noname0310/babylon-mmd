...
AI assistance is generally limited to tab completion or simple code snippet generation.

This is to prevent an meaningless PRs generated by AI.
...
- [nodejs](https://nodejs.org/en/) 24.14.1
...
For set node version to 24.14.1, I recommend using fnm, you can install it using the following command:

you can see the installation instructions here:
[fnm installation](https://github.com/Schniz/fnm#installation) and you also need to shell configure fnm after installation [fnm shell configuration](https://github.com/Schniz/fnm#shell-setup)

Set node version to 24.14.1:

```bash
# You Should run this command in this project directory to set the node version for this project.
fnm use
```

then run `node -v` to check if the node version is correct.

```
node -v
```

You should see `v24.14.1` as the output.
...
When you run `npm start`, the resources in `res/private_test` will not be provided, so there will be errors in the browser.

Due to licensing issues, all files, from common test cases to extreme edge cases, cannot be included in the repository.

Therefore, it may be difficult to perfectly reproduce some tests.
